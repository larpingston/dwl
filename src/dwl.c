...
static void incgaps(const Arg *arg) __attribute__((unused));
...
static void textinputnew(struct wl_listener *listener, void *data);
...
static struct wl_listener new_text_input = {.notify = textinputnew};
...
arrangelayers(Monitor *m)
{
	 ...
	 * Docked (bargaps_*=0): reserve bar_height — standard docking.
	 * Floating: reserve bar_height + bargaps_top so windows
	 * start below the floating bar instead of sliding under it. */
	if (m->scene_buffer->node.enabled) {
		int reserve = m->b.real_height + bargaps_top;
		usable_area.height -= reserve;
		usable_area.y += topbar ? reserve : 0;
	}
...
buttonpress(struct wl_listener *listener, void *data)
{
	...
	case WL_POINTER_BUTTON_STATE_PRESSED:
		...
		if (!c && !exclusive_focus &&
			(node = wlr_scene_node_at(&layers[(bargaps_left > 0 || bargaps_right > 0 || bargaps_top > 0) ? LyrTop : LyrBottom]->node, cursor->x, cursor->y, NULL, NULL)) &&
			(buffer = wlr_scene_buffer_from_node(node)) && buffer == selmon->scene_buffer) {
			cx = (cursor->x - selmon->m.x - bargaps_left) * selmon->wlr_output->scale;
			do
...
createmon(struct wl_listener *listener, void *data)
{
	...
	/* Enable VRR (FreeSync/G-Sync) if the monitor and GPU support it.
	 * This lets the monitor sync its refresh rate to the game's FPS,
	 * eliminating tearing without the input latency of vsync. Silently
	 * ignored if the hardware does not support adaptive sync. */
	...
	/* Floating bar sits above windows; docked bar lives below them */
	m->scene_buffer = wlr_scene_buffer_create(layers[(bargaps_left > 0 || bargaps_right > 0 || bargaps_top > 0) ? LyrTop : LyrBottom], NULL);
...
drawbar(Monitor *m)
{
	...
	if ((w = m->b.width - tw - x) > m->b.height) {
		if (showtitle && c) {
			/* Draw focused window title with SchemeSel highlight */
			drwl_setscheme(m->drw, colors[m == selmon ? SchemeSel : SchemeNorm]);
			drwl_text(m->drw, x, 0, w, m->b.height, m->lrpad / 2, client_get_title(c), 0);
			if (c->isfloating)
				drwl_rect(m->drw, x + boxs, boxs, boxw, boxw, 0, 0);
		} else {
			/* No title (showtitle=0 or no focused window): plain background */
			drwl_setscheme(m->drw, colors[SchemeNorm]);
	...
	{
		int bx, by;
		/* Bar position: respect bargaps_left (horizontal offset) and
		 * bargaps_top (vertical offset from the top/bottom edge)   */
		bx = m->m.x + bargaps_left;
		by = m->m.y + (topbar
			? bargaps_top
			: m->m.height - m->b.real_height - bargaps_top);
		wlr_scene_buffer_set_dest_size(m->scene_buffer,
			m->b.real_width, m->b.real_height);
		wlr_scene_node_set_position(&m->scene_buffer->node, bx, by);
	}
...
focusclient(Client *c, int lift)
{
	...
	if (!c) {
		...
		wl_list_for_each(ti, &text_input_mgr->text_inputs, link) {
			if (ti->seat == seat && ti->focused_surface) {
	...
	wl_list_for_each(ti, &text_input_mgr->text_inputs, link) {
		if (ti->seat != seat) continue;
		if (client_surface(c)->resource && ti->resource &&
				wl_resource_get_client(client_surface(c)->resource) == wl_resource_get_client(ti->resource)) {
			if (ti->focused_surface != client_surface(c)) {
				if (ti->focused_surface)
					wlr_text_input_v3_send_leave(ti);
				wlr_text_input_v3_send_enter(ti, client_surface(c));
				wlr_text_input_v3_send_done(ti);
			}
		} else if (ti->focused_surface) {
			wlr_text_input_v3_send_leave(ti);
			wlr_text_input_v3_send_done(ti);
		}
	}
...
{
	/* Allow async page flips (uncapped FPS) only in fullscreen — keeps the
	 * desktop completely tear-free while games run uncapped. VRR smooths out
	 * any tearing within the monitor's refresh range. Also honours explicit
	 * async requests from native Wayland apps via wp-tearing-control-v1. */
	Client *c = focustop(m);
...
setcfact(const Arg *arg)
{
	...
	if (!arg || !selmon || !selmon->lt[selmon->sellt]->arrange || !sel || sel->isfloating)
		return;
	...
	step = arg->f;
	if (sel_idx != 0 && sel_idx != selmon->nmaster)
		step = -step;
...
setup(void)
{
	...
	status_init(event_loop, stext, sizeof(stext), drawbars,
		status_blocks, sizeof(status_blocks) / sizeof(status_blocks[0]),
		STATUS_INTERVAL);
...
tile(Monitor *m)
{
	int i, n = 0, nm = 0, ns = 0, e;
	int go, gi, mw, sw, my, ty, h;
	int master_total, slave_total;
	...
	e  = (smartgaps == n) ? 0 : m->gaps;
	go = gap_outer * e;
	gi = gap_inner * e;

	if (n > m->nmaster)
		mw = m->nmaster ? (int)roundf((m->w.width - 2*go - gi) * m->mfact) : 0;
	else
		mw = m->w.width - 2*go;

	sw = m->w.width - 2*go - gi - mw;
	...
	master_total = m->w.height - 2*go - (nm > 1 ? gi * (MIN(n, m->nmaster) - 1) : 0);
	slave_total  = m->w.height - 2*go - (ns > 1 ? gi * (ns - 1) : 0);
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			h = (i == nm - 1)
				? m->w.height - go - my
				: (int)roundf(master_total * c->cfact / mfact_sum);
			...
			h = (i == n - 1)
				? m->w.height - go - ty
				: (int)roundf(slave_total * c->cfact / sfact_sum);
			resize(c, (struct wlr_box){
				.x = m->w.x + go + mw + gi,
				.y = m->w.y + ty,
				.width  = sw,
...
dwindle(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < n - 1) {
			if (i % 2 == 0) {
				cw = (i == 0) ? (int)((nw - gi) * m->mfact) : (nw - gi) / 2;
				nx += cw + gi;
				nw -= cw + gi;
			} else {
				ch = (nh - gi) / 2;
...
incgaps(const Arg *arg)
{
	gap_inner += arg->i;
	gap_outer += arg->i;
	if (gap_inner < 0) gap_inner = 0;
	if (gap_outer < 0) gap_outer = 0;
	arrange(selmon);
}
...
updatebar(Monitor *m)
{
	...
	/* Floating bar: scale bargaps into output pixels */
	m->b.width = rw - (int)((bargaps_left + bargaps_right) * m->wlr_output->scale);
	...
	/* bar_padding controls total bar height: font_height + bar_padding */
	m->b.height = m->drw->font->height + bar_padding;
...
void
textinputenable(struct wl_listener *listener, void *data)
{
	/* Handled on commit */
}

void
textinputcommit(struct wl_listener *listener, void *data)
{
	TextInput *input = wl_container_of(listener, input, commit);
	wlr_log(WLR_DEBUG, "text-input: commit");
	if (input->wlr_text_input->resource)
		wlr_text_input_v3_send_done(input->wlr_text_input);
}

void
textinputdisable(struct wl_listener *listener, void *data)
{
	/* Handled on commit */
}

void
textinputdestroy(struct wl_listener *listener, void *data)
{
	TextInput *input = wl_container_of(listener, input, destroy);
...
void
textinputnew(struct wl_listener *listener, void *data)
{
	struct wlr_text_input_v3 *ti = data;
	TextInput *input = ecalloc(1, sizeof(*input));
	input->wlr_text_input = ti;

	input->enable.notify = textinputenable;
	wl_signal_add(&ti->events.enable, &input->enable);
	input->commit.notify = textinputcommit;
	wl_signal_add(&ti->events.commit, &input->commit);
	input->disable.notify = textinputdisable;
	wl_signal_add(&ti->events.disable, &input->disable);
	input->destroy.notify = textinputdestroy;
	wl_signal_add(&ti->events.destroy, &input->destroy);
}
